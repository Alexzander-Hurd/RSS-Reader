...
public class JsonFeedParser : IFeedParser
{
            ...
            var result = new ParsedFeed
            {
                Title =
                    ...
                    && titleEl.ValueKind == JsonValueKind.String
                        ...
                        : null,
                Description =
                    root.TryGetProperty("description", out var descEl)
    ...
    private static ParsedEntry? ParseItem(JsonElement item)
    {
        ...
        if (item.TryGetProperty("url", out var linkEl) && linkEl.ValueKind == JsonValueKind.String)
            ...
            item.TryGetProperty("external_url", out var extEl)
        ...
        if (
            item.TryGetProperty("content_html", out var htmlEl)
            && htmlEl.ValueKind == JsonValueKind.String
        )
            fullContent = htmlEl.GetString();
        else if (
            item.TryGetProperty("content_text", out var textEl)
        ...
        string? summary =
            item.TryGetProperty("summary", out var sumEl) && sumEl.ValueKind == JsonValueKind.String
                ? sumEl.GetString()
                : null;
        ...
        string? dateModified =
            item.TryGetProperty("date_modified", out var modEl)
            && modEl.ValueKind == JsonValueKind.String
                ? modEl.GetString()
                : null;
        string? datePublished =
            item.TryGetProperty("date_published", out var pubEl)
